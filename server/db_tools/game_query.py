...
class TurnQuery(object):
    ...
    def durations_s(self):
        ...
        game_record = GameQuery.QueryFromId(self._game_id).query().record_from_id(self._game_id)
...
class InstructionQuery(object):
    ...
    def fetch(self):
        self._instruction = self._fetch_json(self.INSTRUCTION_FETCH_URL.format(self._base_url, self._instruction_uuid))
        self._moves = self._fetch_json(self.MOVE_FETCH_URL.format(self._base_url, self._instruction_uuid))
        self._live_feedback = self._fetch_json(self.LIVE_FEEDBACK_FETCH_URL.format(self._base_url, self._instruction_uuid))
        return self
...
class GameQuery(object):
    ...
    def query(self):
        if len(self._games) > 0:
            logger.info(f"Already fetched data for games {self._from_id} to {self._to_id}.")
            return self
        ...
        with urllib.request.urlopen(GAME_FETCH_URL) as response:
            ...
            games = [GameRecord.from_json(json.dumps(game_json)) for game_json in games_json]
            games = [game for game in games if (game.id >= self._from_id) and (game.id <= self._to_id)]
            if self._mturk_only:
    ...
    def _per_follower_scatter(self, followers_file):
        ...
        with open(followers_file) as f:
            ...
            followers = [hashlib.md5(follower.encode('utf-8')).hexdigest() for follower in followers]
        ...
        for game in self._games:
            ...
            per_follower_scores[game.follower] = np.append(per_follower_scores[game.follower], game_score)
        ...
        for follower in per_follower_avg_score:
            logger.info(f"Follower {follower} has {per_follower_number_of_games[follower]} games.")
            plt.scatter(per_follower_number_of_games[follower], per_follower_avg_score[follower], label=follower, alpha=0.5)
    ...
    def _score_duration_scatter(self, compare_to_filter_name=""):
        ...
        for game in self._games:
            ...
            if duration.total_seconds() == 0:
                continue
            durations_m = np.append(durations_m, game.parse_duration().total_seconds() / 60)
        ...
        if len(compare_to_filter_name) > 0:
            compare_to_filter_name = GameQuery().filter_by_name(compare_to_filter_name).query()
    ...
    def _pay_rate_histogram(self):
        ...
        for game in self._games:
            if game.leader_rate() > 15:
                logger.info(f"High leader rate ${game.leader_rate()}/hr found in game with score: {game.score} and duration: {game.duration}")
            if game.follower_rate() > 15:
                logger.info(f"High follower rate ${game.follower_rate()}/hr found in game with score: {game.score} and duration: {game.duration}")
        ...
        plt.axvline(np.mean(leader_rates), color='r', linestyle='dashed', linewidth=2, label=f'Mean ({np.mean(leader_rates):0.2f})')
        plt.axvline(np.median(leader_rates), color='b', linestyle='dashed', linewidth=2, label=f'Median ({np.median(leader_rates):0.2f})')
        plt.legend()
        ...
        plt.ylabel("Frequency")
        # Show mean and median.
        plt.axvline(np.mean(follower_rates), color='r', linestyle='dashed', linewidth=2, label=f'Mean ({np.mean(follower_rates):0.2f})')
        plt.axvline(np.median(follower_rates), color='b', linestyle='dashed', linewidth=2, label=f'Median ({np.median(follower_rates):0.2f})')
        plt.legend()
    ...
    def _cost_histogram(self, compare_to_filter_name=""):
        ...
        plt.subplot(1, 2, 1)
        color = colorhash.ColorHash(self._filter_name)
        plt.hist(costs, bins=20, label=f"{self._filter_name} (${costs.sum():0.2f} Total)", color=color.hex)
        plt.title(f"Game Prices (amount paid in total)")
        plt.xlabel("Price ($)")
        plt.ylabel("Frequency")
        # Show mean and median.
        plt.axvline(np.mean(costs), color='r', linestyle='dashed', linewidth=2, label=f'Mean ({np.mean(costs):0.2f})')
        plt.axvline(np.median(costs), color='b', linestyle='dashed', linewidth=2, label=f'Median ({np.median(costs):0.2f})')
        plt.legend()
    ...
    def instant_leave_rate(self):
        ...
        for game in self._games:
            ...
            if duration_s > 0 and duration_s < 60:
                num_instant_leave += 1
        logger.info(f"Instant leave rate: {num_instant_leave}/{len(self._games)} ({num_instant_leave/len(self._games):0.2f}%)")
    ...
    def total_cost(self):
        ...
        logger.info(f"Total cost over {len(self._games)} games: ${total_cost}. Avg cost: ${total_cost / len(self._games)}")
